...
import com.bitwig.extension.controller.api.{ControllerHost, DocumentState, RelativeHardwarControlBindable, SettableEnumValue, SettableRangedValue, Setting}
...
class UserColorSettings(documentState: DocumentState):
  ...
  private val settings: Array[Array[SettableRangedValue]] = Array.ofDim[SettableRangedValue](NUM_USER_BANKS, NUM_KNOBS_PER_BANK)
  private val selector: SettableEnumValue =
    val strings = options.toArray[String]
    val sel = documentState.getEnumSetting("Bank", "Colors", strings, options(0))
    sel.addValueObserver((value: String) => showBank(options.indexOf(value) - 1))
    sel
  ...
  for bank <- 0 until settings.length do
    ...
    for idx <- 0 until settingsBank.length do
      ...
      val colorSetting = documentState.getNumberSetting(label, "Colors", 0, 125, 1, null, 0)
...
object UserColorSettings:
   ...
   */
  def createTarget(host: ControllerHost, setting: SettableRangedValue): RelativeHardwarControlBindable =
