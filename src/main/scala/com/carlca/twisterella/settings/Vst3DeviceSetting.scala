...
/** VST3 specific device settings.
  ...
  * construction.
  */
class Vst3DeviceSetting private (
    id: String,
    params: mutable.Map[String, Integer]
) extends AbstractDeviceSetting[String, Integer](id, params):
...
object Vst3DeviceSetting:
  /** Constructs a Vst3DeviceSetting object from a TOML table.
    ...
    * @return
    *   A new Vst3DeviceSetting. Throws on errors.
    */
  def fromToml(table: TomlTable): Vst3DeviceSetting =
    ...
    if id.length != 32 then
      throw IllegalArgumentException(
        "VST3 device ID must be 32 characters long"
      )
