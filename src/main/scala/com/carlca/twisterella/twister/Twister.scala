...
class Twister(extension: TwisterellaExtension):
  ...
  def setActiveBank(index: Int): Unit =
    ...
    activeBank = index
    midiOut.sendMidi(
      ShortMidiMessage.CONTROL_CHANGE + MidiChannel.SYSTEM,
      activeBank,
      127
    )
  ...
  private def createHardware(extension: TwisterellaExtension): Unit =
    ...
    val sideButtonsFirstRightCC =
      sideButtonsFirstLeftCC + Twister.Bank.NUM_LEFT_SIDE_BUTTONS
    ...
    for bank <- 0 until banks.length do
      ...
      for knob <- 0 until hardwareKnobs.length do
        ...
        val rgbLight =
          LightMidiInfo(MidiChannel.BUTTON, MidiChannel.RGB_ANIMATION, cc)
        val ringLight =
          LightMidiInfo(MidiChannel.ENCODER, MidiChannel.RING_ANIMATION, cc)
  ...
  /** Create a listener for bank switch messages.
    *
    * Updates the internal active bank index and shows a popup notification if
    * popups are enabled.
    */
  private def createBankSwitchListener(midiIn: MidiIn): Unit =
    midiIn.setMidiCallback((status, data1, data2) =>
      boundary {
        ...
        if status != 0xb3 || data1 > 3 || data2 != 0x7f then break()
...
object Twister:
  ...
  class Bank:
    ...
    val leftSideButtons: Array[TwisterButton] =
      Array.ofDim[TwisterButton](NUM_LEFT_SIDE_BUTTONS)
    val rightSideButtons: Array[TwisterButton] =
      Array.ofDim[TwisterButton](NUM_RIGHT_SIDE_BUTTONS)
